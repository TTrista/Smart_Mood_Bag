...
#define SYSTEM_DURATION    (60UL * 60UL * 1000UL)
#define HRV_DELAY          (10UL * 60UL * 1000UL)
...
void loop() {
  ...
  if (millis() - lastBLEUpdate >= 5000) {
    ...
    if (!isnan(HR) && !isnan(HRV) && !isnan(sig_data) &&
      ...
      String textData = String(HR, 1) + "," + String(HRV, 1) + "," + String(sig_data, 3);
      hrCharacteristic->setValue(textData.c_str());
      hrCharacteristic->notify();
      Serial.print("📤 BLE Sent: ");
      Serial.println(textData);
    } else {
      Serial.println("⚠️ Invalid data skipped.");
    }
  }
...
void updateNeoPixel() {
  if (peak) {
    ...
    pixels.setBrightness(millis() - systemStartTime > 120000UL ? 10 : 100);
  } else {
